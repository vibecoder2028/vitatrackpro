...
// Mock data with proper TypeScript-like structure
const mockData = {
  stores: [
...
function VitaTrackPro() {
  ...
  const MetricCard = useCallback(({ icon: Icon, title, value, change, color, subtitle }) => (
          ...
          <div key={spg.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              <button 
                ...
                aria-label={`More options for ${spg.name}`}
              >
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
              <div className="flex gap-2">
                <button 
                  ...
                  aria-label={`View details for ${spg.name}`}
                >
                  View Details
                </button>
                <button 
                  ...
                  aria-label={`Process payout for ${spg.name}`}
                >
  ...
  ), [formatCurrency, StatusBadge]);

  // Payouts view component
  const PayoutsView = useCallback(() => (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex gap-2">
          <button 
            ...
            aria-label="Export payout report"
          >
          ...
          <button 
            ...
            aria-label="Process all pending payouts"
          >
  ...
  const StoresView = useCallback(() => {
    const filteredStores = mockData.stores.filter(store => 
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        <button 
                          ...
                          aria-label={`Process payout for ${store.name}`}
                        >
  ...
  }, [selectedRegion, formatCurrency, StatusBadge]);

  // Reports view component
  const ReportsView = useCallback(() => (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
  ...
  const renderView = useCallback(() => {
    switch (activeTab) {
      ...
      case 'spg': return <SPGView />;
      case 'payouts': return <PayoutsView />;
      case 'stores': return <StoresView />;
      case 'reports': return <ReportsView />;
      default: return <DashboardView />;
    }
  }, [activeTab, DashboardView, InventoryView, SPGView, PayoutsView, StoresView, ReportsView]);
